...
export default function ItemOverview() {
  ...
  const item = useSelector(state => 
    state.items.flatMap(category => category.items).find(item => item.id === itemId)
  );

  const itemdata = useSelector(state => state.items);
  const categoryId = useSelector(state => state.categories);

  const items = itemdata.find(category => 
    category.items.some(item => item.id === itemId)
  );

  const category = categoryId.find(category => category.id === items?.categoryId);
  ...
  return (
    <div className='px-5'>
      <div>
        <Link to={`/category/${category.id}`}>{category.category}</Link> / {item.name}
      </div>
      <div className='flex flex-col md:flex-row gap-4 w-full py-5'>
        ...
        <div className='flex flex-col md:w-[50%] gap-2'>
          ...
          <div className='flex flex-row gap-2'>
            ...
            <span className='border-2 border-solid border-black flex items-center justify-center px-4 py-2'>
              {quantity}
            </span>
            <button 
              className='border-2 border-solid border-black px-2' 
          ...
          {quantity >= item.quantity && <span className='text-red-600'>Stock Sold</span>}
        </div>
      </div>
    </div>
  );
